...
trajectory_file <- read.csv("Cape Hallett.csv", header=TRUE, sep=",")
...
max(density)
m <- max(density)
contour(longitude, latitude, t(density), levels=c(0.1,0.25,0.5,1,1.5,2),xlim=c(150,175), ylim=c(-82,-70))
...
p1 <-  ggplot(density_df, aes(x=longitude, y=latitude, fill=density)) +
  ...
  labs(x=NULL, y=NULL, fill = "Density(%)", title = "                                      HYSPLIT of Cape Hallett") +
  coord_cartesian(xlim = c(150, 175), ylim = c(-82, -70)) +
  theme_minimal() +
  geom_polygon(data = map_data("world"), aes(x = long, y = lat, group = group), color = "black", size= 1.1, fill = NA) +
  theme(panel.border = element_rect(color = "black", fill = NA, size = 3)) +
  theme(panel.grid.minor = element_blank()) +
  theme(axis.text.x = element_text(size = 14, face ="bold"), 
        axis.text.y = element_text(size = 14, face ="bold")) +
  theme(legend.position="none") +
  scale_x_continuous(breaks = c(150,155,160,165,170), expand = c(0, 0), labels = c('150¡ãE','155¡ãE', '160¡ãE','165¡ãE','170¡ãE')) +
  scale_y_continuous(breaks = c(-80,-78,-76,-74,-72,-70), expand = c(0, 0), labels = c('80¡ãS','78¡ãS','76¡ãS','74¡ãS','72¡ãS','70¡ãS')) +
  geom_point(x=170.22, y=-72.32, color="black", size=5, shape=21, fill="black")
p1

ggsave("Cape Hallett2 .pdf", plot = p1, device = "pdf", width = 16, height = 22, units = "cm", dpi = 600, family = "Times")
