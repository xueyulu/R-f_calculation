...
trajectory_file <- read.csv("JRI.csv", header=TRUE, sep=",")
...
max(density)
m <- max(density)
contour(longitude, latitude, t(density), levels=c(0.1,0.25,0.5,1,2,4), xlim=c(-70,-50), ylim=c(-70,-58))
...
p1 <-  ggplot(density_df, aes(x=longitude, y=latitude, fill=density)) +
  ...
  labs(x=NULL, y=NULL,fill = "Density(%)",
       title = "                                      HYSPLIT of James Ross Island") +
  ...
  geom_point(x=-57.75, y=-64.17, color="black", size=5, shape=21, fill="black")
p1

ggsave("JRI1.pdf", plot = p1, device = "pdf", width = 16, height = 22, 
